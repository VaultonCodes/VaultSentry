import type { AxiosAdapter, AxiosPromise, AxiosRequestConfig, InternalAxiosRequestConfig } from 'axios';
 ...
 * 检查 URL 是否应该被排除
 */
const shouldExcludeUrl = (url: string, excludeUrls?: Array<string | RegExp | ((url: string) => boolean)>): boolean => {
...
export const createThrottleAdapter = (
  ...
  const opts: ThrottleAdapterOptions = typeof options === 'number' ? { threshold: options } : options;
    ...
    if (onThrottle && typeof onThrottle === 'function') {
      ...
      try {
        ...
        import('@/utils/message')
          .then(({ MsgWarning }) => {
            MsgWarning(throttleTipMessage, false, 2000);
          })
          .catch(() => {
 ...
 * ```
 */
export const applyThrottleAdapter = (instance: any, options?: ThrottleAdapterOptions | number): void => {
