...
// 生成水印画布
const generateWatermarkCanvas = (str: string, font: string, textColor: string): string => {
...
const createGlobalWatermark = (config: any) => {
  ...
  Object.assign(container.style, {
    ...
    backgroundImage: `url(${generateWatermarkCanvas(config.text, config.font, config.textColor)})`,
...
const createLocalWatermark = (el: HTMLElement, config: any) => {
  ...
  // 应用新水印
  el.style.backgroundImage = `url(${generateWatermarkCanvas(config.text, config.font, config.textColor)})`;
