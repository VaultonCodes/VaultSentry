...
const handleChange = async (file: any) => {
  ...
  const acceptTypeList = list.map((item: string) => {
    return getType(item);
  });
  // 如果要检索的字符串值没有出现，则该方法返回 -1
  const isString = acceptTypeList.filter((item: string) => {
    return rawFile.type.includes(item);
  });
  // 用于校验是否符合上传条件
  ...
  <div>
    ...
    <div style="margin-top: 6px">
      <div
        ...
        :key="item.url"
        class="template-file m-t-2px rounded-6px text-#555 hover:bg-[--el-color-primary-light-9] dark:text-#CFD3DC"
      >
        <ElIcon size="16" style="margin-right: 5px"><Link /></ElIcon>
