<script setup lang="ts" name="VaultUploadImages">
...
import { NoticeError, NoticeSuccess, NoticeWarning } from '@/utils/message.ts';
...
const beforeUpload: UploadProps['beforeUpload'] = rawFile => {
  ...
  if (!imgType) NoticeWarning('上传图片不符合所需的格式');
  if (!imgSize)
    setTimeout(() => {
      NoticeWarning(`上传图片大小不能超过 ${props.fileSize}M！`);
    }, 0);
...
const handleHttpUpload = async (options: UploadRequestOptions) => {
  ...
  try {
    const res: any = await request.upload(props.action, formData);
...
/** 文件数超出 */
const handleExceed = () => {
  NoticeWarning(`当前最多只能上传 ${props.limit} 张图片，请移除后上传！`);
};
...
<template>
  <div class="upload-box">
    <ElUpload
      ...
      class="upload"
      :class="[imageDisabled ? 'disabled' : '', drag ? 'no-border' : '']"
      :multiple="true"
    ...
    <ElImageViewer v-if="imgViewVisible" :url-list="[viewImageUrl]" @close="imgViewVisible = false" />
  </div>
</template>
