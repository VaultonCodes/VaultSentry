...
import { add, batchDelete, deleteById, getById, listPage, update, updateStatus } from '@/api/system/notice/index.ts';
...
const handleSwitch = (row: any) => {
  const text = row.noticeStatus === '1' ? '启用' : '停用';
  ...
  <div class="vault-flex">
    <VaultCard>
      ...
      >
        ...
        <ElTableColumn label="序号" prop="noticeId" width="120px" align="center" type="index"></ElTableColumn>
        <ElTableColumn
          label="公告名称"
          prop="noticeTitle"
          width="260px"
        ...
        ></ElTableColumn>
        <ElTableColumn label="公告类型" prop="noticeType" width="100px" align="center">
          <template #default="scope">
            <VaultTag :tag-options="noticeOptions" :value="scope.row.noticeType"></VaultTag>
          </template>
        </ElTableColumn>
        <!-- 注意：如果后端数据返回的是字符串"0" OR "1"，这里的active-value AND inactive-value不需要加冒号，会认为是字符串，否则：后端返回是0 AND 1数字，则需要添加冒号 -->
        <ElTableColumn label="公告状态" prop="noticeStatus" width="100px" align="center">
          <template #default="scope">
        ...
        <ElTableColumn label="修改人" prop="updateBy" width="180px" align="center"></ElTableColumn>
        ...
        <template #content>
          <ElForm ref="formRef" :rules="rules" :model="form" label-width="80px" status-icon>
            <ElRow>
              ...
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="公告备注" prop="remark">
                  <ElInput v-model="form.remark" :rows="5" type="textarea" placeholder="请输入公告备注" />
                </ElFormItem>
              </ElCol>
            </ElRow>
          </ElForm>
