...
import { add, batchDelete, deleteById, getById, listPage, update, updateStatus } from '@/api/system/user/index.ts';
import { assignUserRole, listNormalRole } from '@/api/system/role/index.ts';
...
const handleListPage = async () => {
  ...
  //     VauleDatePicker(searchParams.value, dateRange.value)
  //   );
...
const handleStaticPage = () => {
  listPage(datePicker(searchParams.value, dateRange.value));
};
...
const handleSwitch = (row: any) => {
  const text = row.userStatus === '1' ? '启用' : '停用';
...
<template>
  <div class="vault-flex">
    <VaultSearch :is-expanded="true" :show-search="showSearch">
      <!-- 搜索条件 -->
      <ElForm :inline="true">
        ...
        <ElFormItem label="登录时间" prop="loginTime">
          <ElDatePicker
            v-model="dateRange"
            type="datetimerange"
    ...
    <VaultCard>
      ...
      <ElRow :gutter="10">
        ...
        <ElCol v-auth="['system:role:update']" :span="1.5">
          <ElButton type="primary" icon="edit" plain :disabled="single" @click="handleAssignRoles()">分配角色</ElButton>
        </ElCol>
        <ElCol v-auth="['system:role:export']" :span="1.5">
          <ElButton type="warning" icon="download" plain>导出</ElButton>
        </ElCol>
        <ElCol v-auth="['system:role:import']" :span="1.5">
          <ElButton type="info" icon="upload" plain>导入</ElButton>
        </ElCol>
      ...
      >
        ...
        <ElTableColumn label="序号" prop="userId" width="80px" align="center" type="index"></ElTableColumn>
        <ElTableColumn
          label="登录账号"
          prop="loginName"
          width="120px"
        ...
        ></ElTableColumn>
        <ElTableColumn label="头像" prop="avatar" width="80px" align="center">
          <template #default="scope">
            <div class="flex flex-justify-center">
              ...
              >
                <template #error>
                  <ElIcon class="text-[--el-color-primary]" :size="36">
                    <CircleCloseFilled />
                  </ElIcon>
                </template>
              </ElImage>
        ...
        <ElTableColumn
          ...
          :show-overflow-tooltip="true"
        ></ElTableColumn>
        <ElTableColumn label="用户类型" prop="userType" width="100px" align="center">
          <template #default="scope">
            <VaultTag :tag-options="userTypeOptions" :value="scope.row.userType"></VaultTag>
          </template>
        </ElTableColumn>
        <ElTableColumn label="用户性别" prop="sex" width="100px" align="center">
          <template #default="scope">
        ...
        <ElTableColumn label="用户状态" prop="userStatus" width="100px" align="center">
          <template #default="scope">
          ...
          <ElForm ref="formRef" :rules="rules" :model="form" label-width="80px" status-icon>
            <ElRow>
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="用户名称" prop="userTitle">
                  <ElInput v-model="form.userTitle" placeholder="请输入用户名称" clearable />
                </ElFormItem>
              </ElCol>
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="用户类型" prop="userType">
                  <ElSelect v-model="form.userType" placeholder="请选择用户类型" clearable>
                    <ElOption
              ...
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="用户状态" prop="userStatus">
                  <ElSelect v-model="form.userStatus" placeholder="请选择用户状态" clearable>
              ...
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="用户头像" prop="avatar">
                  <KoiUploadImage v-model:image-url="form.avatar">
                    <template #content>
              ...
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="手机号" prop="phone">
                  <ElInput v-model="form.phone" placeholder="请输入手机号"></ElInput>
                </ElFormItem>
              </ElCol>
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="用户备注" prop="remark">
                  <ElInput v-model="form.remark" :rows="5" type="textarea" placeholder="请输入用户备注" />
                </ElFormItem>
              </ElCol>
            </ElRow>
          </ElForm>
      ...
      <KoiDialog ref="koiDialogRef" title="分配角色" :height="450" :loading="confirmLoading" :footer-hidden="true">
        <template #content>
          <div class="flex flex-justify-center">
            <ElTransfer
