...
const tableList = ref<any>([
  {
    ...
    operLocation: '',
    operParam:
      '{"pageNo":1,"pageSize":10,"loginName":null,"ipAddress":null,"loginStatus":null,"beginTime":null,"endTime":null}',
    jsonResult:
  ...
  <div class="koi-flex">
    <KoiCard>
      ...
      <ElForm v-show="showSearch" :inline="true">
        ...
        <ElFormItem label="IP地址" prop="operIp">
          <ElInput
        ...
        <ElFormItem label="操作人员" prop="operMan">
          <ElInput
      ...
      >
        ...
        <ElTableColumn label="序号" prop="operId" width="70px" align="center" type="index"></ElTableColumn>
        <ElTableColumn
          label="操作名称"
          prop="operName"
          width="180px"
      ...
      <KoiDialog ref="koiDialogRef" :title="title" top="6vh" :height="660" :width="800" :footer-hidden="true">
        <template #content>
          ...
          <ElDescriptions direction="vertical" :column="3" border>
            <ElDescriptionsItem label="方法名称">{{ form.methodName }}</ElDescriptionsItem>
            <ElDescriptionsItem label="消耗时间[毫秒]">{{ form.costTime }}</ElDescriptionsItem>
            <ElDescriptionsItem label="操作状态">
              <ElTag :type="form.operStatus == '0' ? '' : form.operStatus == '1' ? 'danger' : 'warning'">
                {{ form.operStatus == '0' ? '操作成功' : form.operStatus == '1' ? '操作失败' : '未知状态' }}
              </ElTag>
            </ElDescriptionsItem>
            <ElDescriptionsItem v-if="form.operStatus == '1'" label="异常信息" :span="3">
              {{ form.costTime }}
            </ElDescriptionsItem>
            <ElDescriptionsItem label="请求参数" :span="3">{{ form.operParam }}</ElDescriptionsItem>
