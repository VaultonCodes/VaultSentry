...
import { add, batchDelete, deleteById, getById, listPage, update, updateStatus } from '@/api/system/role/index.ts';
import { listMenuIdsByRoleId, listMenuNormal, saveRoleMenu } from '@/api/system/menu/index.ts';
  ...
  <div class="vault-flex">
    <VaultCard>
      ...
      <ElForm v-show="showSearch" :inline="true">
        ...
        <ElFormItem label="角色状态" prop="roleStatus">
          <ElSelect
      ...
      <ElRow :gutter="10">
        ...
        <ElCol v-auth="['system:role:import']" :span="1.5">
          <ElButton type="success" icon="Postcard" plain :disabled="single" @click="handleAssignMenu()">
            分配菜单
          </ElButton>
        </ElCol>
        <ElCol v-auth="['system:role:xxx']" :span="1.5">
          <ElButton type="info" icon="upload" plain>测试</ElButton>
        </ElCol>
      ...
      >
        ...
        <ElTableColumn label="序号" prop="roleId" width="120px" align="center" type="index"></ElTableColumn>
        <ElTableColumn
          label="角色名称"
          prop="roleName"
          width="120px"
          ...
          align="center"
        ></ElTableColumn>
        <ElTableColumn label="角色编号" prop="roleCode" width="120px" align="center"></ElTableColumn>
        <!-- 注意：如果后端数据返回的是字符串"0" OR "1"，这里的active-value AND inactive-value不需要加冒号，会认为是字符串，否则：后端返回是0 AND 1数字，则需要添加冒号 -->
        <ElTableColumn label="角色状态" prop="roleStatus" width="100px" align="center">
        ...
        <ElTableColumn label="角色排序" prop="sorted" width="100px" align="center"></ElTableColumn>
        <ElTableColumn
          label="角色备注"
          prop="remark"
          width="260px"
        ...
        <template #content>
          <ElForm ref="formRef" :rules="rules" :model="form" label-width="80px" status-icon>
            ...
            <ElRow :gutter="10">
              <ElCol :sm="{ span: 12 }" :xs="{ span: 24 }">
                <ElFormItem label="角色状态" prop="roleStatus">
                  <ElSelect v-model="form.roleStatus" placeholder="请选择角色状态" style="width: 260px" clearable>
                    <ElOption label="启用" value="1" />
                    <ElOption label="停用" value="0" />
                  </ElSelect>
                </ElFormItem>
              ...
              <ElCol :sm="{ span: 12 }" :xs="{ span: 24 }">
                <ElFormItem label="角色排序" prop="sorted">
            ...
            <ElRow>
              <ElCol :sm="{ span: 24 }" :xs="{ span: 24 }">
                <ElFormItem label="角色备注" prop="remark">
                  <ElInput v-model="form.remark" :rows="5" type="textarea" placeholder="请输入角色备注" />
                </ElFormItem>
              </ElCol>
            </ElRow>
          </ElForm>
