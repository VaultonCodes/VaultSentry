...
<template>
  <div class="user-container">
    ...
    >
      <template #reference>
        <ElImage class="user-avatar h-32px w-32px select-none rounded-full" :src="avatar"></ElImage>
      </template>

      <!-- 用户信息卡片内容 -->
      <div class="user-card-content">
        <div class="user-card-header">
          <ElImage class="h-36px w-36px select-none rounded-full" :src="avatar"></ElImage>
          <div class="user-info">
        ...
        <div class="user-card-menu">
          <div class="user-menu-item" @click="handleCommand('mine')">
            <ElIcon :size="15"><User /></ElIcon>
            <span>个人中心</span>
          </div>
        </div>
        <div class="user-card-footer">
          <ElButton icon="SwitchButton" plain @click="handleCommand('logout')">
            退出登录
          </ElButton>
        </div>
      </div>
