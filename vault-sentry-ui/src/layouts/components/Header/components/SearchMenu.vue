...
const menuList: any = computed(() =>
  authStore.menuList.filter((item: any) => item.meta.isVisible == '1' && item.meta.parentId != '0')
);
...
<template>
  ...
  <div style="position: relative; display: flex; align-items: center">
    <div
      class="vault-icon vault-pulse-i h-36px w-36px flex flex-justify-center flex-items-center rounded-md hover:bg-[--el-header-toolbar-icon-hover-bg-color]"
      @click="handleMenuOpen"
    >
      <ElTooltip content="搜索菜单">
        <VaultGlobalIcon name="vault-search" size="18" />
      </ElTooltip>
    </div>
    ...
    >
      <div style="display: flex; flex-direction: column; height: 500px">
        ...
        <!-- 搜索结果列表 -->
        <div v-if="searchList.length" ref="menuListRef" style="flex: 1; overflow-y: auto; padding: 10px 20px">
          <div
          ...
          >
            <div style="display: flex; align-items: center; flex: 1; gap: 12px">
              <VaultGlobalIcon v-if="item.meta.icon" :name="item.meta.icon" size="18" :style="getIconStyle(item)" />
              <ElIcon v-else :style="getIconStyle(item)">
                <Menu />
              </ElIcon>
        ...
        <div
          ...
          "
        >
          <ElButton size="small" plain>↑↓ 选择</ElButton>
          <ElButton size="small" plain>↵ 确认</ElButton>
          <ElButton size="small" plain>ESC 关闭</ElButton>
        </div>
      </div>
    </ElDialog>
  </div>
